   ...
   # Check that the analysis name is not empty and get it
	...
	if (tclvalue(quantnfvar)==1) {
		if (apex=="0"){
			...
			assign(tclvalue(outvar), MS.DataCreation(path=path,mz=mz,DataType="Agilent",apex=TRUE, quant=TRUE), envir=.GlobalEnv)
			tkmessageBox(title="check",message= "Done",icon="info",type="ok")
		}else{
			
			assign(tclvalue(outvar), MS.DataCreation(path=path,mz=mz,DataType="Agilent",apex=FALSE, quant=TRUE), envir=.GlobalEnv)
		...
		if (apex=="0"){
			...
			assign(tclvalue(outvar), MS.DataCreation(path=path,mz=mz,DataType="Agilent",apex=TRUE, quant=FALSE), envir=.GlobalEnv)
			tkmessageBox(title="check",message= "Done",icon="info",type="ok")
		}else{
			
			assign(tclvalue(outvar), MS.DataCreation(path=path,mz=mz,DataType="Agilent",apex=FALSE, quant=FALSE), envir=.GlobalEnv)
			tkmessageBox(title="Done",message= "Done",icon="info",type="ok")
		}
		}
	}
